...
def main():
    ...
    while True:
        ...
        if tvecs_list and x and y and z and detected and dist and angle is not None:
            ...
            avg_data = [float(avg_x), float(avg_y), float(avg_z), float(-avg_angle), 0, 0, float(avg_dist)]
            cv2.putText(frame, f" X = {float(avg_x):.4}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            cv2.putText(frame, f" Y = {float(avg_y):.4}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            cv2.putText(frame, f" Z = {float(avg_z):.4}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            cv2.putText(frame, f" Angle = {float(-avg_angle):.4}", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            cv2.putText(frame, f" Distance between fingertips = {float(avg_dist/1000):.4}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        ...
        cv2.imshow('Hand Pose Estimation', output)
